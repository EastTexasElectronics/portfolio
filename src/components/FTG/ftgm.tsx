...
const FileTreeGenerator = () => {
    ...
    const GitHubButton = ({ href, text }: { href: string; text: string }) => (
        <a href={href} className="inline-flex items-center justify-center text-white font-bold py-2 px-4  mx-auto bg-gradient-to-bl from-fuchsia-600 via-violet-600 to-blue-600 rounded-lg cursor-pointer hover:opacity-90 transition-opacity duration-200 border-r-10" target="_blank" rel="noopener noreferrer" onClick={() => track('Button Pressed', { button: text, location: 'FileTreeGenerator' })}>
                ...
                <nav aria-label="Table of contents" className="bg-gray-700 p-6 rounded-lg shadow-inner">
                    ...
                    <ul className="grid grid-cols-2 gap-4 mb-8 list-none pl-0">
                        {['overview', 'features', 'languages', 'installation', 'usage', 'contributing'].map((item) => (
                            <li key={item}>
                                <Button asChild variant="link" size="lg" className="w-full bg-gray-600 hover:bg-gray-500 transition-colors duration-200" onClick={() => track('Button Pressed', { button: item, location: 'Table of Contents' })}>
                    ...
                    <section key={section} id={section} aria-labelledby={`${section}-heading`} className="bg-gray-700 p-6 rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300">
                        ...
                        {section === 'languages' && (
                            <div className="grid grid-cols-2 gap-4 mt-4">
                                {languages.map((lang) => (
                                    <a key={lang.name} href={`https://github.com/EastTexasElectronics/File-Tree-Generator-Multiverse/tree/main/${lang.path}`} target="_blank" rel="noopener noreferrer" className="bg-gray-600 hover:bg-gray-500 text-white font-bold py-2 px-4 rounded transition-colors duration-200 text-center" onClick={() => track('Button Pressed', { button: lang.name, location: 'Languages' })}>
                                        {lang.name}
                                    </a>
                                ))}
                            </div>
